...
-- Populate varification tables
INSERT INTO verify
VALUES('database_services', (
    SELECT COUNT(*)
    FROM database_services
), 5);
INSERT INTO verify
...
CREATE OR ALTER TRIGGER trigger_insert
ON sales
FOR INSERT
AS
...
INTO @service_id, @new_amount;

WHILE @@FETCH_STATUS = 0
BEGIN
    UPDATE database_services
    SET sales_ytd += @new_amount
...
FETCH NEXT FROM mCursor
INTO @service_id, @old_amount;

WHILE @@FETCH_STATUS = 0
BEGIN
    UPDATE database_services
    SET sales_ytd -= @old_amount
    WHERE service_id = @service_id;
...
CREATE OR ALTER TRIGGER trigger_update
ON sales
FOR UPDATE
AS
DECLARE @service_id INT;
...
SELECT i.service_id, i.amount, d.amount
...
FETCH NEXT FROM mCursor
INTO @service_id, @new_amount, @old_amount;

WHILE @@FETCH_STATUS = 0
BEGIN
    UPDATE database_services
    SET sales_ytd += (@new_amount - @old_amount)
    WHERE service_id = @service_id;
...
INSERT INTO sales
VALUES(16, '2020-09-15', 100000, 103);
