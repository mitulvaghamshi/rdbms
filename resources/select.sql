...
SELECT SUBSTRING(TC.TABLE_NAME, 1, 15) [Table],
SUBSTRING(CCU.COLUMN_NAME, 1, 20) [Column],
SUBSTRING(TC.CONSTRAINT_NAME, 1, 35) [Constraint],
SUBSTRING(TC.CONSTRAINT_TYPE, 1, 11) [Type],
COALESCE(SUBSTRING(CC.CHECK_CLAUSE, 1, 75), SUBSTRING(KCU.TABLE_NAME, 1 , 15) + '(' + SUBSTRING(KCU.COLUMN_NAME, 1, 20) + ')', 'Missing') [Constraint_Details]
...
PRINT 'SELECT 4 - Show the Master table data'
SELECT *
FROM dbo.firebase_services;

PRINT 'SELECT 5 - Show the Sales table data'
SELECT *
FROM sales;
